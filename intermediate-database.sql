...
2. SELECT i.invoie_date, c.first_name, c.last_name, i.total
FROM invoice i 
...
SELECT t.name
FROM track t
JOIN playlist_ track pt ON p.track_id = t.track_id
JOIN album a ON t.album_id = a.album_id
 ...
 4. SELECT *
 FROM track
 WHERE genre_id IN (SELECT genre_id FROM genre WHERE comedy = 'Comedy');
